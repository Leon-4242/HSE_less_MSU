...
bool arecompact(int *arr, int n, Graph g, int * ver) {
	...
	for (int i = 1; i <= n;  i++) {
		if (arr[i-1] == 2) {
			if (g.num_adjacent(i) == 2 ) {
				array = g.adjacent(i);
				if (!g.areAdjacent(array[0], array[1])) {
					*ver = i;
					delete[] array;
					return true;
				}
...
void compact(Graph *g, int *arr, int n) {
	...
	while (arecompact(arr, n, *g, &curr)) {
		...
		g->addEdge(array[0], array[1]);
		g->setWeight(1+g->Weight(array[0], curr)+g->Weight(array[1], curr), array[0], array[1]);
		g->delVertex(curr);
		delete[] array;
	}
}
