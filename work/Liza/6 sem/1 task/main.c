...
int main(int argc, char** argv) {
	...
	char *filename = NULL; 
	double *local_array = NULL, *local_rev = NULL, *buffer = NULL; 
	int* ind = NULL;
	...
	output_file_mpi("out_data_e.txt", r, r, n, local_rev, rank, size);	

	MPI_Barrier(MPI_COMM_WORLD);
	t1 = MPI_Wtime();

	flag = jordan_reverse_mpi(n, local_array, local_rev, rank, size, buffer, ind);
