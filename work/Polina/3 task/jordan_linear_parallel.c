...
void* jordan_linear_parallel(void* arg) {
	...
	for (k = 0; k < args->n; ++k) {
		...
		local_n_max = k; local_max = fabs(args->a[args->ind[k]*args->n + k]);
		...
		for (i = start; i < args->n; i += args->numThreads) {
			if (local_max < fabs(args->a[args->ind[i]*args->n + k])) {
				local_n_max = i; local_max = fabs(args->a[args->ind[i]*args->n + k]);
			}
		}
		...
		pthread_barrier_wait(args->barrier);
		if (args->thread_id == 0) {
			args->b[args->ind[k]] /= args->a[args->ind[k]*args->n + k];
		...
		for (i = args->thread_id; i < args->n; i += args->numThreads) {
			if (i != k) {
				for (j = k+1; j < args->n; ++j) {
					args->a[args->ind[i]*args->n+j] -= args->a[args->ind[i]*args->n+k]*args->a[args->ind[k]*args->n+j]; 
				}
			}
		}
		...
		for (i = args->thread_id; i < args->n; i += args->numThreads) {
			if (i != k) {
				args->b[args->ind[i]] -= args->a[args->ind[i]*args->n+k]*args->b[args->ind[k]]; 
			}
		}
		pthread_barrier_wait(args->barrier);
	}
